    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notes.update", [$note->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $note->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $note->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('claims') ? 'is-invalid' : '' }}" name="claims[]" id="claims" multiple required>
                    @foreach($claims as $id => $claim)
                        <option value="{{ $id }}" {{ (in_array($id, old('claims', [])) || $note->claims->contains($id)) ? 'selected' : '' }}>{{ $claim }}</option>
                    @endforeach
                </select>
