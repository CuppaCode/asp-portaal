...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.certificateCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.certificate-categories.update', $certificateCategory->id) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.certificateCategory.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $certificateCategory->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', $certificateCategory->duration ?? 12) }}">
            ...
            <div class="form-group">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="enable_notifications" id="enable_notifications" value="1" {{ old('enable_notifications', $certificateCategory->enable_notifications ?? true) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notify_days_before') ? 'is-invalid' : '' }}" type="number" name="notify_days_before" id="notify_days_before" value="{{ old('notify_days_before', $certificateCategory->notify_days_before ?? 30) }}" min="1" max="365">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reminder_frequency_days') ? 'is-invalid' : '' }}" type="number" name="reminder_frequency_days" id="reminder_frequency_days" value="{{ old('reminder_frequency_days', $certificateCategory->reminder_frequency_days ?? 7) }}" min="1" max="30">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notification_recipients') ? 'is-invalid' : '' }}" type="text" name="notification_recipients" id="notification_recipients" value="{{ old('notification_recipients', is_array($certificateCategory->notification_recipients) ? implode(', ', $certificateCategory->notification_recipients) : '') }}" required>
